...
import static dev.ultreon.devicesnext.filesystem.Constants.LOGGER;
import static dev.ultreon.devicesnext.filesystem.Disk.BLOCK_SIZE;

public class FSDirectory implements FSNode {
    private final dev.ultreon.devicesnext.filesystem.Disk disk;
    ...
    public FSDirectory(Disk diskIn, FileSystem fsIn, FSDirectory parentIn, long addressIn) {
        ...
        buffer.get(1, dst);
        if (b > dst.length) {
            LOGGER.warn("File name too long: " + b + " > " + dst.length);
            dst = new byte[b];
        }
    ...
    public void rename(String name, FSNode node, boolean flush) {
        this.children.remove(this.name);
        this.children.put(name, node);
        if (node instanceof FSFile file) {
            file.name = name;
        } else if (node instanceof FSDirectory directory) {
            directory.name = name;
        } else {
            throw new FileSystemIoException("Unknown node type");
        }
    ...
    public void move(String name, FSDirectory other, boolean flush) {
        children.remove(name);
        other.children.put(name, children.get(name));
